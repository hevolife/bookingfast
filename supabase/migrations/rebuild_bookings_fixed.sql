/*
  # Reconstruction complète de la table bookings - VERSION CORRIGÉE
  
  1. Sauvegarde des données
  2. Suppression complète de la table
  3. Recréation propre
  4. Restauration avec colonnes explicites
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bookings_backup') THEN
...
-- 8. Restaurer les données avec colonnes EXPLICITES
...
SELECT 
  ...
  COALESCE(transactions, '[]'::jsonb),
  COALESCE(booking_status, 'pending'),
  assigned_user_id,
  notes,
  custom_service_data,
  google_calendar_event_id,
...
BEGIN
  ...
  RAISE NOTICE 'Nombre de réservations restaurées: %', booking_count;
END $$;
